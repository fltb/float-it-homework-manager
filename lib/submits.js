...
const submitManager = {
    ...
    async toExcel(user, problem) {
        ...
        for (let i = 0; i < submits.length; i++) {
            ...
            datas.push([prob.title, usr.realName, usr.enrollmentYear, usr.classNumber, usr.loginName, submit.updatedAt])
        };

        const buffer = xlsx.build([{ name: "提交名单", data: datas }]);
        const pth = path.join(__dirname, "..", "提交名单.xlsx");
        ...
        for (let i = 0; i < allProblemList.length; i++) {
            ...
            for (let i = 0; i < lackUsrList.length; i++) {
                if (lackUsrList[i]) {
                    ...
                    datas.push([problem_.title, usr.realName, usr.enrollmentYear, usr.classNumber, usr.loginName,]);
                }
            }
        }
        const buffer = xlsx.build([{ name: "缺交名单", data: datas }]);
        const pth = path.join(__dirname, "..", "缺交名单.xlsx");
        await fse.writeFile(pth, buffer);
        return;
    }
}
