...
const userManager = {
    ...
    checkingInfos(infos, ignore){
        if (!loginNameRe.test(infos.loginName) && !(ignore && infos.loginName === undefined)) {
            throw new Error("登录名应当为大小写字母或者数字, 不能少于 3 位或大于 20 位。")
        }
        if (!passwordRe.test(infos.password) && !(ignore && infos.password === undefined)) {
            throw new Error("密码的位数太短了吧, 至少要 6 位");
        }
        if (!enrollmentYear.test(infos.enrollmentYear) && !(ignore && infos.enrollmentYear === undefined)) {
            throw new Error("你确定你的年份" + infos.enrollmentYear + "没有啥问题?");
        }
        if (!classNumber.test(infos.classNumber) && !(ignore && infos.classNumber === undefined)) {
            throw new Error("什么奇奇怪怪的班级:" + infos.classNumber);
        }
        if (!realname.test(infos.realName) && !(ignore && infos.realName === undefined)) {
    ...
    async ResetUser(id, infos, ingore) {
        this.checkingInfos(infos, ingore);
